...
// This is the alt. firmware version made by TheSlowGrowth which added CLOCK DIV
// It is here now modified by Batchs/Scrotum Lab
// So now all settings with CLOCK DIV mode can be saved + I added MIDI mod from Gritty-Grids
...
struct Options {
  ...
  uint8_t pack() const {
    ...
    if (output_mode == OUTPUT_MODE_CLOCK_DIV) {
      byte |= 0x03;
    }
    if (!swing) {
      byte |= 0x08;
  ...
  void unpack(uint8_t byte) {
    ...
    if (byte & 0x40) {
      output_mode = OUTPUT_MODE_DRUMS;
    } else if (byte & 0x03) {
      output_mode = OUTPUT_MODE_CLOCK_DIV;
    } else {
      output_mode = OUTPUT_MODE_EUCLIDEAN;
    }
